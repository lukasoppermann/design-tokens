...
In the settings you can defined the valid alias identifiers like `ref`. By default it is set to `alias, ref, reference`.
...
In the output JSON generated by the variables, the ones marked with `hide from publish` are excluded by default. By turning this option on, they will be included.
...
When enabled the json file will be minified meaning whitespace and line breaks will be removed.
...
If you which to ignore specific styles, use the [exclusion prefix](https://github.com/lukasoppermann/design-tokens/blob/main/README.md#exclusion-prefix).
...
When enabled the json that is end to the server will be minified meaning whitespace and line breaks will be removed.
...
3. You may have changed the `exclusion prefix` setting to `false` which means only styles that include the prefix e.g. `_colors/blue` are exported
