 ...
 */
const buildFigmaData = async (figma: PluginAPI, settings: Settings): Promise<figmaDataType> => {
  ...
  return {
    ...
    paintStyles: getPaintStyles(await figma.getLocalPaintStylesAsync()).filter(item => filterByPropertyName(item, userExclusionPrefixes)),
    gridStyles: getGridStyles(await figma.getLocalGridStylesAsync()).filter(item => filterByPropertyName(item, userExclusionPrefixes)),
    textStyles: getTextStyles(await figma.getLocalTextStylesAsync()).filter(item => filterByPropertyName(item, userExclusionPrefixes)),
    effectStyles: getEffectStyles(await figma.getLocalEffectStylesAsync()).filter(item => filterByPropertyName(item, userExclusionPrefixes))
  }
}
