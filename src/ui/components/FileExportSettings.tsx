...
export const FileExportSettings = () => {
  ...
  const handleFormSubmit = (event) => {
    ...
    if (exportSettingsForm.checkValidity() === true) {
      ...
      console.log('settings: ', pluginSettings.keyInName)
  ...
  return (
    <form onSubmit={(event) => handleFormSubmit(event)} className={style}>
      ...
      <a
        ref={downloadLinkRef}
        download={`${settings.filename ?? 'design-tokens'}${settings.extension ?? '.json'}`}
        title={`${settings.filename ?? 'design-tokens'}${settings.extension ?? '.json'}`}
